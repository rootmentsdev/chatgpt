...
import botImage from '../assets/ai-chat.png';
          ...
          <Form.Group className="mb-3">
            ...
            >
              <Form.Control
                as="textarea"
                rows={1}
                placeholder="Ask a question or upload feedback"
                value={combinedInput}
                onChange={(e) => setCombinedInput(e.target.value)}
                style={{
                  border: 'none',
                ...
                <>
                  ...
                  {selectedIssue !== null && (
                    <>
                      <hr />
                      <h6>⚠️ Root Cause</h6>
                      <p>{result.rootCauses?.[selectedIssue] || 'No root cause available.'}</p>

                      <h6>✅ Action Plan</h6>
                      <ul>
                        {result.actionPlan?.[selectedIssue]?.length > 0
                          ? result.actionPlan[selectedIssue].map((step, i) => <li key={i}>{step}</li>)
                          : <li>No suggestions available</li>}
                      </ul>
                    </>
                  )}
